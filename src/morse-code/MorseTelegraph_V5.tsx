          ...
          <div className={`glass-bar rounded-2xl shadow-lg ${dark?"border border-white/10":"border border-white/40"}`}>
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button className={`px-3 py-2 rounded-md text-sm border ${isPlaying?(dark?"bg-sky-600 text-white border-sky-600 hover:bg-sky-500":"bg-sky-700 text-white border-sky-700 hover:bg-sky-600"):buttonBase}`} onClick={()=> (isPlaying? stopPlayback() : playTranscriptNow())} disabled={!transcript.trim()} title="Play back with tone (uses current speed)">{isPlaying?"Stop":"Play"}</button>
              </div>
            </div>
          </div>
        </div>
