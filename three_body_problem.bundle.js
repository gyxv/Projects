var ThreeBodyGlassSim = (() => {
  ...
  function ThreeBodyGlassSim() {
    ...
    const [eventType, setEventType] = (0, import_react.useState)(null);
    const [eventBodyInfo, setEventBodyInfo] = (0, import_react.useState)("");
    const [countdown, setCountdown] = (0, import_react.useState)(120);
    const [hexColors, setHexColors] = (0, import_react.useState)(["#cccccc", "#cccccc", "#cccccc"]);
    const [chosenDuration, setChosenDuration] = (0, import_react.useState)(null);
    const [progressLines, setProgressLines] = (0, import_react.useState)([]);
    const [candidateInfo, setCandidateInfo] = (0, import_react.useState)("");
    const [attemptInfo, setAttemptInfo] = (0, import_react.useState)("");
    const [orientation, setOrientation] = (0, import_react.useState)(null);
    const [zoom, setZoom] = (0, import_react.useState)(defaultSettings.zoom);
    const orientationRef = (0, import_react.useRef)(orientationPresets[0]);
    const [pan, setPan] = (0, import_react.useState)([0, 0]);
    const panRef = (0, import_react.useRef)([0, 0]);
    const followRef = (0, import_react.useRef)(null);
    const shatterPosRef = (0, import_react.useRef)([[0, 0], [0, 0], [0, 0]]);
    const draggingRef = (0, import_react.useRef)(false);
    const dragStartRef = (0, import_react.useRef)([0, 0]);
    const panStartRef = (0, import_react.useRef)([0, 0]);
    const postEventRef = (0, import_react.useRef)(false);
    const rocketRef = (0, import_react.useRef)(null);
    const [showSpeed, setShowSpeed] = (0, import_react.useState)(false);
    const [showGravity, setShowGravity] = (0, import_react.useState)(false);
    const seedRef = (0, import_react.useRef)("");
    const [copied, setCopied] = (0, import_react.useState)(false);
    const [importOpen, setImportOpen] = (0, import_react.useState)(false);
    const [seedInput, setSeedInput] = (0, import_react.useState)("");
    const seedImportRef = (0, import_react.useRef)(null);
    const outerObjectsRef = (0, import_react.useRef)(generateRegion([0, 0]));
    const regionCentersRef = (0, import_react.useRef)([[0, 0]]);
    ...
    const radius = 0.035;
    const softEps = 1e-4;
    const targetScaleRef = (0, import_react.useRef)(180);
    const scaleRef = (0, import_react.useRef)(180);
    const userZoomRef = (0, import_react.useRef)(defaultSettings.zoom);
    const preBufRef = (0, import_react.useRef)(null);
    const liveRef = (0, import_react.useRef)({
      p: [[0, 0], [0, 0], [0, 0]],
      v: [[0, 0], [0, 0], [0, 0]],
      tSim: 0
    });
    const mapRef = (0, import_react.useRef)({ realStart: 0, baseSpeed: 1 });
    const [speedMul, setSpeedMul] = (0, import_react.useState)(1);
    const trailsRef = (0, import_react.useRef)([[], [], [], []]);
    const [trailMax, setTrailMax] = (0, import_react.useState)(90);
    const [panelOpen, setPanelOpen] = (0, import_react.useState)(true);
    const eventIndexRef = (0, import_react.useRef)(-1);
    const shardsRef = (0, import_react.useRef)([]);
    const destroyedRef = (0, import_react.useRef)([false, false, false]);
    const pausedRef = (0, import_react.useRef)([false, false, false, false]);
    const collisionHandledRef = (0, import_react.useRef)(false);
    ...
    function drawScene(ctx, p) {
      ...
      if (showGravity) {
        ...
        const infMap = /* @__PURE__ */ new Map();
        const consider = (pos, mass2, target) => {
          const r = sub(pos, target);
          const d2 = r[0] * r[0] + r[1] * r[1] + 1e-6;
          const strength = mass2 / d2;
          const key = pos.join(",");
          const existing = infMap.get(key);
          if (!existing || existing.strength < strength) infMap.set(key, { pos, mass: mass2, strength });
        };
      ...
      if (rocketRef.current) {
        ...
        if (r.power > 0) {
          ...
          const grad2 = ctx.createLinearGradient(-4, 0, -4 - len, 0);
          grad2.addColorStop(0, "#ffffaa");
          grad2.addColorStop(0.4, "#ff8800");
    ...
    const loopRef = (0, import_react.useRef)(() => {
    });
    loopRef.current = () => {
      ...
      if (isPlaying) {
        ...
        if (simTimeTarget <= tEvent) {
          ...
          while (dtLeft > 1e-6) {
            ...
            const prevP = liveRef.current.p.map((x) => [...x]);
