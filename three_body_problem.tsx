...
export default function ThreeBodyGlassSim() {
  ...
  const importSeedRef = useRef<Seed | null>(null);
  ...
  async function preSimulateAndSetup(opts?: { targetTEvent?: number; targetRealTime?: number; seed?: Seed }) {
    let codes: string[] = [];
    if (opts?.seed) {
      codes = opts.seed.hex;
      setHexColors(codes);
    ...
    async function runCandidate(eps: number, ang: number) {
    ...
    let best: { buffer: any; tEvent: number; kind: "collision" | "ejection"; info: string; eps: number; ang: number } | null = null;
    ...
    if (!best) {
      ...
      seedRef.current = {
        ...
        p: orientationRef.current.label === "Random" ? orientationRef.current.p : undefined,
        v: orientationRef.current.label === "Random" ? orientationRef.current.v : undefined,
        eps: best.eps,
        ang: best.ang,
        hex: codes,
        tEvent: best.tEvent,
        duration: opts?.targetRealTime ?? chosenDuration ?? 0,
      };
    }
      ...
      if (simTimeTarget <= tEvent) {
        ...
        while (dtLeft > 1e-6) {
          ...
          if (rocketRef.current) {
            ...
            r.v = add(r.v, mul(acc, step)) as [number, number];
            r.angle += r.turn * 1.5 * step;
            r.p = add(r.p, mul(r.v, step)) as [number, number];
          }
          for (const sh of shardsRef.current) {
  ...
  useEffect(() => {
    ...
    preSimulateAndSetup({
      ...
      seed: importSeedRef.current || undefined,
    });
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key === "0" && rocketRef.current) { followRef.current = 3; return; }
      if (e.key === "1" || e.key === "2" || e.key === "3") { followRef.current = parseInt(e.key) - 1; return; }
  ...
  function handleImportSeed() {
    try {
      const obj = JSON.parse(atob(seedInput.trim())) as Seed;
      if (obj.label === "Random" && obj.p && obj.v) {
        const rand: OrientationPreset = { label: "Random", p: obj.p, v: obj.v };
  ...
  return (
    <div className="relative w-full h-[88vh] md:h-[92vh] bg-black text-white font-sans overflow-hidden rounded-2xl shadow-2xl" onWheel={handleWheel}>
      ...
      <div className="absolute top-4 right-4 px-4 py-3 rounded-2xl backdrop-blur-xl bg-white/10 border border-white/20 shadow-lg">
        ...
        <button onClick={copySeed} className="mt-2 px-2 py-1 text-xs rounded-lg bg-white/10 hover:bg-white/20 border border-white/20">Copy seed</button>
      </div>
      ...
      {!isReady && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="px-6 py-4 rounded-2xl backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl text-center relative">
            ...
            <button onClick={() => setShowSeedImport(true)} className="absolute bottom-2 right-2 px-2 py-1 text-xs rounded-lg bg-white/10 border border-white/20">Skip Exploration</button>
          </div>
          {showSeedImport && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="px-6 py-4 rounded-2xl backdrop-blur-xl bg-white/20 border border-white/30 shadow-2xl text-center">
                <div className="mb-2">Paste seed</div>
                <input className="w-64 px-2 py-1 mb-3 text-black" value={seedInput} onChange={(e)=>setSeedInput(e.target.value)} />
                <div className="flex justify-end gap-2">
                  <button onClick={() => setShowSeedImport(false)} className="px-3 py-1 text-xs rounded-lg bg-white/10 border border-white/20">Cancel</button>
                  <button onClick={handleImportSeed} className="px-3 py-1 text-xs rounded-lg bg-white/10 border border-white/20">Import simulation</button>
                </div>
              </div>
            </div>
          )}
