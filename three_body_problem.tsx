...
export default function ThreeBodyGlassSim() {
  ...
  const followRef = useRef<number | "rocket" | null>(null);
    ...
    try {
      ...
      if (preBufRef.current) {
        ...
        if (preBufRef.current.states.length > 0) {
          ...
          liveRef.current = { p: st.p.map((x) => [...x]) as any, v: st.v.map((x) => [...x]) as any, tSim: 0 };
        }
      }
      mapRef.current.realStart = performance.now() / 1000;
      trailsRef.current = [[], [], []];
  ...
  return (
    <div className="relative w-full h-[88vh] md:h-[92vh] bg-black text-white font-sans overflow-hidden rounded-2xl shadow-2xl" onWheel={handleWheel}>
      ...
      <div className="absolute top-4 right-4 px-4 py-3 rounded-2xl backdrop-blur-xl bg-white/10 border border-white/20 shadow-lg">
        ...
        <button onClick={copySeed} className="mt-2 text-xs px-2 py-1 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20">Copy seed</button>
      </div>
      ...
      {!isReady && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative px-6 py-4 rounded-2xl backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl text-center">
            ...
            <button onClick={() => setShowImport(true)} className="absolute bottom-2 right-2 text-xs px-2 py-1 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20">Skip Exploration</button>
          </div>
          {showImport && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="px-6 py-4 rounded-2xl backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl text-center">
                <div className="mb-2">Paste seed</div>
                <textarea value={seedInput} onChange={(e) => setSeedInput(e.target.value)} className="w-64 h-24 mb-3 rounded-lg bg-black/20 border border-white/30 text-white p-2" />
                <button onClick={importSeed} className="px-3 py-1 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20">Import simulation</button>
              </div>
            </div>
          )}
        </div>
