...
const MPH_PER_UNIT = 10000; // arbitrary conversion factor
...
export default function ThreeBodyGlassSim() {
  ...
  function topGravitySources(p: [number, number][], W: number, H: number) {
  ...
  function distortPoint(pt: [number, number], infl: { pos: [number, number]; mass: number }[]) {
  ...
  function drawScene(ctx: CanvasRenderingContext2D, p: [number, number][]) {
    ...
    const drawSpeedBox = (x: number, y: number, speed: number) => {
      ...
      if (simTimeTarget <= tEvent) {
        ...
        while (dtLeft > 1e-6) {
          ...
          if (rocketRef.current) {
            ...
            if (r.thrustUp) r.thrustPower = Math.min(1.2, r.thrustPower + 1.0 * step);
            if (r.thrustDown) r.thrustPower = Math.max(0, r.thrustPower - 1.0 * step);
